...
inline std::size_t hash_boost_shared_ptr_creris_credit_risk_time_series_configuration(const boost::shared_ptr<creris::credit_risk::time_series_configuration>& v){
    std::size_t seed(0);
    combine(seed, *v);
    return seed;
}

inline std::size_t hash_std_vector_boost_shared_ptr_creris_credit_risk_time_series_configuration_(const std::vector<boost::shared_ptr<creris::credit_risk::time_series_configuration> >& v){
    std::size_t seed(0);
    for (const auto i : v) {
        combine(seed, hash_boost_shared_ptr_creris_credit_risk_time_series_configuration(i));
    }
    return seed;
}
...
std::size_t model_configuration_hasher::hash(const model_configuration&v) {
    std::size_t seed(0);

    combine(seed, v.name());
    combine(seed, v.description());
    combine(seed, v.entity());
    ...
    combine(seed, hash_std_vector_boost_shared_ptr_creris_credit_risk_time_series_configuration_(v.time_series_configuration_set()));
