...
std::size_t adjustment_constraint_hasher::hash(const adjustment_constraint&v) {
    std::size_t seed(0);

    combine(seed, hash_std_vector_creris_credit_risk_time_series_unversioned_key(v.related_series()));
    combine(seed, v.constraint_operator());
    combine(seed, v.constraint_amount());
